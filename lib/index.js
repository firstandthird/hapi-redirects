...
module.exports = function(server, options, next) {
  ...
  var redirector = function(request, reply) {
    ...
    _.each(request.params, function(param, paramName){
      redirectTo = redirectTo.replace("{" + paramName + "}", param).replace("{" + paramName + "?}", param).replace("{" + paramName + "*}", param);
    })
    if(redirectTo) {
      if (log) {
        server.log(['hapi-redirects'], { routePath: routePath, from: request.path, to: redirectTo });
      }
